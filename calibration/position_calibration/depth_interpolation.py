...
class DepthInterpolation():
    def __init__(self, arg, date, front_peak_illum_idx, mid_peak_illum_idx, mid2_peak_illum_idx, mid3_peak_illum_idx, back_peak_illum_idx):
        ...
        self.max_data_mid2 = np.load('./calibration/position_calibration/2023%s/npy_data/max_data_%s.npy'%(date, "mid2"))
        self.max_data_mid3 = np.load('./calibration/position_calibration/2023%s/npy_data/max_data_%s.npy'%(date, "mid3"))
        ...
        for m in range(len(self.m_list)):
            for w in range(len(self.wvl_list)):   
                for idx, i in enumerate(self.sample_pts_flatt):
                    ...
                    # interp_depth = self.cubic_interpolation(new_depth_range, depth_range, all_position_peak_illum_idx[:,m,w,i].reshape(len(depth_range), -1), 4)[idx_start:idx_end+1]
