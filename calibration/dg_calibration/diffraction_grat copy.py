...
class PixelRenderer():
    ...
    def get_grid_points_undistort(self, illum_dir, n_pattern):
        ...
        illum_grid_points = np.load(os.path.join(illum_dir, "pattern_%02d.npy" %n_pattern)).reshape(-1,1,2).astype(np.float32)
...
if __name__ == "__main__":    
    ...
    # depth dir
    depth_dir = "./calibration/gray_code_depth/spectralon_depth.npy"
    # illum dir 
    illum_dir = "./calibration/dg_calibration/test_2023_05_15_17_34_patterns"
    ...
    opt_param = torch.tensor([ 1.5, 0.5,  0.0033,  0.], dtype= torch.float, requires_grad=True, device= arg.device)
    ...
    for i in range(epoch):        
        for n in range(1,2):
            ...
            processed_points = torch.tensor(point_process.point_process(arg, total_dir, pattern_dir, wvls, n),device=arg.device, dtype= torch.long)
            ...
            if n == 1:
                uv_cam, real_uv = uv_cam[:,0], real_uv[:,0] # only m = -1 orders
            else:
                uv_cam, real_uv = uv_cam[:,2], torch.flip(real_uv[:,2], dims = (2,)) # only m = 1 orders
                
            # if epoch % 1000 == 0:
            #     renderer.visualization(uv_cam, real_uv)
            
            if n == 1:
                loss_patt = loss_f(uv_cam.to(torch.float32), real_uv.to(torch.float32))
                loss = loss_patt
            else:
                # loss += loss_patt
                loss_patt = loss_f(uv_cam.to(torch.float32), real_uv.to(torch.float32))
