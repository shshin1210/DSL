% Thi is the without interpolation (without white balancing)
%---------------------------------------------------------
...
%AT = subcamt(:,(3:end))'; % with WB
AT = subcamt(:,(2:end))'; % without WB
WH = subcamt(:,2)';
for i=1:(size(AT,1))
%   ATd(i,:) = AT(i,:)./WH;
   ATd(i,:) = AT(i,:);
...
%BT = subradt(:,(3:end))'; % with WB
BT = subradt(:,(2:end))'; % without WB
WH = subradt(:,2)';
for i=1:(size(BT,1))
%   BTd(i,:) = BT(i,:)./WH;
   BTd(i,:) = BT(i,:);
