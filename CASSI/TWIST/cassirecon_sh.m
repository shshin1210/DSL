function im_spec = cassirecon_sh(im_observed_rgb, shifted_ind, spec2rgb, params)
%% Prepare Parameters
...
y = im_observed_rgb; % observed image: in our case RGB image
% [n1, n2, ~] = size(im_observed_rgb); % row, column

QPhi = @(x)compute_A_mul_any_v3(x, shifted_ind, spec2rgb);
QPhi_T= @(y)compute_AT_mul_any_v3(y, shifted_ind, spec2rgb);
