...
subplot(2,2,3); imagesc(cData('Velocity.Y')); title('Velocity.Y'); colorbar
subplot(2,2,4); imagesc(cData('depth.Z')); title('depth.Z'); colorbar
...
cData = exrreadchannels('x35_3a.exr');
cData = containers.Map({'R', 'G', 'B'}, {R, G, B}); % here R, G, B are data imagess
exrwritechannels('test_zip.exr','zip','half',cData);
