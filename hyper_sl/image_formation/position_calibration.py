...
class PositionCalibration():
    def __init__(self):
        ...
        self.depth_idx = np.arange(0, 301, 1)
    ...
    #     # depth, m, wvl, sample points
    #     depth_peak_illum_idx = depth_peak_illum_idx.reshape(len(self.depth_arange), len(self.m_list), len(self.wvl_list), -1)
    ...
    def interpolate_image(self, depth_peak_illum_idx):
        ...
        # depth, m, wvl, sample points
        depth_peak_illum_idx = depth_peak_illum_idx.reshape(len(self.depth_arange), len(self.m_list), len(self.wvl_list), -1)
        depth_peak_illum_idx = depth_peak_illum_idx.reshape(-1, len(self.sample_pts_flatt))
        
        # depth, m, H, W, wvl
        # m = +1 xy interpolation
        depth_peak_illum_idx_image = np.zeros(shape=(len(self.depth_arange)*len(self.m_list)*len(self.wvl_list), self.cam_H, self.cam_W))
        
        for i in range(len(self.depth_arange)*len(self.m_list)*len(self.wvl_list)):
            depth_peak_illum_idx_intp = self.interpolation(self.sample_pts, depth_peak_illum_idx[i])
            depth_peak_illum_idx_image[i] = depth_peak_illum_idx_intp
    ...
    def full_wavelength_interpolation(self, depth_peak_illum_idx):
        ...
        # np.save(os.path.join(self.npy_dir, 'depth_peak_illum_idx_image_2.npy'), depth_peak_illum_idx_image)
        depth_peak_illum_idx_image = np.load(os.path.join(self.npy_dir, 'depth_peak_illum_idx_image_2.npy')).reshape(len(self.depth_arange), len(self.m_list), len(self.wvl_list), self.cam_H, self.cam_W)
        
        # depth peak illum idx image m = -1, m = 1 분리하기
        ...
        first_illum_idx = np.round(first_illum_idx).astype(np.int16)
        ...
        for i in range(self.cam_H *self.cam_W):
            if (0 in first_illum_idx[:,:,i]) or (317 in first_illum_idx[:,:,i]):                
                ...
                if len(depth_idx_0) * len(depth_idx_317) == 0:
                    ...
                    other_depths_idx = np.delete(self.depth_idx, depth_unique_val)
                    interval_array_none = np.linspace(first_illum_idx[other_depths_idx,0,i], first_illum_idx[other_depths_idx,-1,i], 47)
                    interval_array_none = np.round(interval_array_none).astype(np.int16)
                    first_illum_idx_final[other_depths_idx,:,i] = interval_array_none.T
                    ...
                    zero_depth_unique = depth_unique_val
                    ...
                    # for depths with none of 0 or 317
                    other_depths_idx = np.delete(self.depth_idx, np.concatenate((depth_unique_val, zero_depth_unique)))
