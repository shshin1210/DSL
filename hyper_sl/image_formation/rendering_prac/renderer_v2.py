...
class PixelRenderer():
    ...
    def __init__(self, arg):
        ...
        self.xyz1, self.uv1, self.proj_center = self.proj_sensor_plane()
        ...
        self.virtual_xyz1_dg = torch.stack((self.sensor_X_virtual, self.sensor_Y_virtual, self.sensor_Z_virtual, torch.ones_like(self.sensor_X_virtual, device = 'cuda')), dim = 2)
        self.virtual_xyz1_proj = self.extrinsic_diff @ self.virtual_xyz1_dg
        
        # # save mat files
        # sensor_X_real = self.uv1[0]
        # sensor_Y_real = self.uv1[1]
        ...
        #         savemat(os.path.join(self.dat_path, 'dispersion_coordinates_m%d_wvl%d.mat'%(self.m_list[m], lmb[i]*1e9 )), {'x': self.virtual_xyz1_proj[m,i,0].data.cpu().numpy(), 'y':self.virtual_xyz1_proj[m,i,1].data.cpu().numpy(), 'xo': sensor_X_real.data.cpu().numpy(), 'yo': sensor_Y_real.data.cpu().numpy()})
    ...
    def render(self, depth, normal, hyp, occ, cam_coord, eval, illum_opt = None):
        ...
        # dg coord to rproj coord
        xyz_proj_real = self.proj.dg_to_rproj_v2(xyz_dg, self.extrinsic_diff)       
        
        # distortion coefficent 불러오기
        sensor_X_virtual_distorted, sensor_Y_virtual_distorted = self.dist.distort_func(xyz_proj_real[..., 0,:]/self.proj_pitch, xyz_proj_real[...,1,:]/self.proj_pitch, self.p_list[...,0,:], self.p_list[...,1,:])
        
        uv1_proj = torch.stack((sensor_X_virtual_distorted, sensor_Y_virtual_distorted), dim=3)
        
        c_proj, r_proj = uv1_proj[:,:,:,0], uv1_proj[:,:,:,1]

        cr_proj = torch.cat((c_proj.unsqueeze(dim = 3), r_proj.unsqueeze(dim =3)), dim = 3)
        cr_proj = cr_proj.transpose(4,3).reshape(self.batch_size, self.m_n, self.wvls_n, self.pixel_num, 2) 
        cr_proj = cr_proj.to(self.device)  # TODO: do we need this?   
        
        c_proj, r_proj = cr_proj[...,1], cr_proj[...,0]

        cond = (0<= c_proj)*(c_proj < self.proj_H)*(0<=r_proj)*(r_proj< self.proj_W)
        c_proj_valid, r_proj_valid = c_proj[cond], r_proj[cond]
        c_proj_valid, r_proj_valid = torch.tensor(c_proj_valid), torch.tensor(r_proj_valid)  # TODO: do we need this? 

        new_idx = self.proj_W * c_proj_valid.long() + r_proj_valid.long()  # TODO: use a variable instead of hard coding? 
        ...
        a = cam_N_img[0].reshape(768,1024,42,3).detach().cpu().numpy()
        import matplotlib.pyplot as plt
        fig, ax= plt.subplots()
        plt.ylim(0, self.cam_H)
        plt.imshow(a[:,:,0]*100)
        plt.show()
        
        return cam_N_img, xy_proj_real_data, illum_data, shading
    ...
    def proj_sensor_plane(self):
        ...
        uv1 = torch.stack((c,r,ones), dim = 0)
        xyz = (torch.linalg.inv(self.intrinsic_proj_real)@(uv1*self.proj_focal_length))
...
if __name__ == "__main__":
    ...
    # plane_XYZ = loadmat('C:/Users/owner/Documents/GitHub/Scalable-Hyperspectral-3D-Imaging/hyper_sl/image_formation/plane_XYZ.mat')['XYZ_q'][562,343]
    
    # depth = torch.zeros(size=(pixel_num, )).unsqueeze(dim =0)
    # depth[:] = torch.tensor(plane_XYZ)[2]*1e-3
    ...
    # cam_coord = torch.tensor([[343,561,1]]).unsqueeze(dim =0)
    ...
    plane_XYZ = torch.tensor(loadmat('C:/Users/owner/Documents/GitHub/Scalable-Hyperspectral-3D-Imaging/hyper_sl/image_formation/plane_XYZ.mat')['XYZ_q'])
    ...
    import cv2
    illum = cv2.imread("C:/Users/owner/Documents/GitHub/Scalable-Hyperspectral-3D-Imaging/dataset/image_formation/illum/gird_720.png").astype(np.float32)
    illum = illum / 255.
    illum = torch.tensor(illum, device='cuda').unsqueeze(dim = 0)
